// Amaç: Yerel ve uzak sunuculardaki nadirlik bilgilerini (EventPokemon) güncellemek ve veri tabanına işlemek.
...
class RarityUpdater private constructor(
    ...
    suspend fun seedLocalEvents() {
        ...
        if (seeded.events.isNotEmpty()) {
            eventDao.upsertEventPokemonAll(seeded.events.map { it.toEntity("asset_seed") })
        }
    }
    ...
    suspend fun refreshFromRemote() {
        runCatching {
            ...
            if (updateFile.events.isNotEmpty()) {
                eventDao.upsertEventPokemonAll(updateFile.events.map { it.toEntity("github_updates") })
            }
        }.onFailure { error ->
...
private fun EventPokemonSeed.toEntity(sourceName: String): EventPokemonEntity {
    fun parseDate(value: String?): Date? = DateParseUtils.parseIsoDate(value)
