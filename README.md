...
- Android Studio (Hedgehog+ veya uyumlu)
- JDK 17
- Android SDK 35
- ADB (cihaz testi icin)
...
`local.properties` dosyasinda en az `sdk.dir` tanimli olmalidir.

## Calistirma

Android Studio'da projeyi acip `app` modulunu `debug` olarak calistirabilirsiniz.
CLI ile derleme:
...
- Durum: aktif Android scanner projesi, telemetry + living DB + event/costume baglami entegre.
- Son bilinen surum: `1.10.0` (`gradle.properties`).
- Son commit (yerel): `43f4a9d5` (bu calisma oncesi).
- Not: event/costume/shiny ayristirma kalitesi telemetry verisiyle iyilestirilmeye devam ediyor.
